...
# --- 開始日〜月度末までの営業日数（休業日・土日を除く）を自動計算 ---
            ...
            try:
                ...
                if factory_mode == "本社":
                    ...
                    for recipe_code, line_set in grp_lines.items():
                        if len(line_set) > 1:
                            speeds = {ln: get_sp(ln, 12, factory_mode) for ln in line_set}
                            fastest_line = max(speeds, key=speeds.get)
                            mask = (df_final['中身設計コード'] == recipe_code) & (df_final['製造ライン'].isin(UNIFY_CANDIDATES))
                            df_final.loc[mask, '製造ライン'] = fastest_line
